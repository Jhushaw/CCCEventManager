...
class EventsService{
	...
	public function unattendEvent($eventID, $currentAttendies){
		MyLogger::info("Entering EventsService.unattendEvent()");
		//database connection variables stored in config/database.php file
		$servername = config("database.connections.mysql.host");
		$port = config("database.connections.mysql.port");
		$username = config("database.connections.mysql.username");
		...
		$service = new EventsDAO($db);
		$flag = $service->unattendEvent($eventID, $currentAttendies);
		
		//in PDO you "close" the database connection by setting the PDO object to null
		$db = null;
		
		//Return the results
		MyLogger::info("Exit editService.unattendEvent() with " . $flag);
		return $flag;
	}
	...
	public function eventsAttending() {
		MyLogger::info("Entering EventsService.eventsAttending()");
		//database connection variables stored in config/database.php file
		$servername = config("database.connections.mysql.host");
		$port = config("database.connections.mysql.port");
		$username = config("database.connections.mysql.username");
		...
		// get all current logged in users events using the data service
		$service = new EventsDAO($db);
		$flag = $service->eventsAttending();
